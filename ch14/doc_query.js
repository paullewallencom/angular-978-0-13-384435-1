...
function findItems(err, words){
    ...
    words.find({size:{$gt: 12}}, function(err, cursor){
        displayWords("Wordslonger than 12 characters: ", cursor);
    });
    ...
    words.find({$and: [{first:{$in: ['a','e''i','o','u']}},
                       {last:{$in: ['a','e','i','o','u']}]},
            function(err, cursor){
        displayWords("Words that startand end with a vowel: ", cursor);
        });
    ...
    words.find({otherChars: {$exist:true}}, function(err, cursor){
        displayWords("Words with all 5 vowels: ", cursor);
    });
